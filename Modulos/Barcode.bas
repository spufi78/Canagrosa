...
Sub DrawBarcode(ByVal muestra As Long, obj As Control, tipo As String, tamano As Integer)
    ...
    obj.CurrentY = 1
    If tamano = 0 Then
        obj.Print "Número:" & omuestra.CodigoParticular(muestra)
    Else
...
'    obj.Print "Número:" & omuestra.getID_GENERAL & "/" & omuestra.getANNO & "   Fecha: " & Format(omuestra.getFECHA_RECEPCION, "dd/mm/yyyy")
    If tamano > 0 Then
        obj.Print "Número:" & omuestra.CodigoParticular(muestra) & "    Fecha: " & Format(omuestra.getFECHA_RECEPCION, "dd/mm/yyyy")
    End If
