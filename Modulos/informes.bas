...
Public Function enviar_informe(ByVal MUESTRA As Long, EDICION As Integer, manejador As Long) As Boolean
    ...
    ' Se indica en el correo los parametros que están fuera de rango
    Dim rs As ADODB.Recordset
    Dim oDET As New clsDeterminaciones
    Set rs = oDET.lista_determinaciones_fuera_rango(MUESTRA)
    Dim ASUNTO As String
    If rs.RecordCount > 0 Then
        body = body & vbNewLine & vbNewLine & "ATENCION : Los siguientes parámetros están fuera de rango : " & vbNewLine & vbNewLine
        Do
            If alerta <> "" Then
                alerta = alerta & ", "
            End If
            alerta = alerta & rs(0)
            body = body & " - Parámetro : " & rs(0) & ", Resultado : " & rs(1) & " " & rs(2) & vbNewLine
            rs.MoveNext
        Loop Until rs.EOF
    End If
    Set oDET = Nothing
            ...
            If rs.RecordCount > 0 Then
                ...
                If Dir(origen) <> "" Then
                    ...
                    If rs(7) > 1 Then
                        ...
                        XLS.Hyperlinks.Add XLS.Cells(fila, COLS.INFORME_2), "../Documentos/Análisis/" & rs(6) & "/AN-" & rs(5) & "-" & rs(6) & "-Ed" & CStr(EDICION) & ".pdf"
                    End If
                Else
...
'                End Select
    ...
    altura_error = "Abriendo Excel Linea/Baño"
...
'    XLA.Visible = True
    ...
    End If
        ...
        altura_error = "Generando resultados últimas muestras"
...
genera_excel_OR_Error:
    ...
    log "Error " & Err.Number & " (" & Err.Description & ") in procedure genera_excel_OR of Módulo informes"
