...
Begin VB.Form frmInformeMuestrasAnalizadasPorAnalistaDeterminacion 
   BackColor       =   &H00C0C0C0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Informe de determinaciones realizadas por Analista"
   ClientHeight    =   9585
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12435
   Icon            =   "frmInformeMuestrasAnalizadasPorAnalistaDeterminacion.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin VB.CommandButton cmdMarcar 
      ...
      Height          =   870
      Left            =   60
      Picture         =   "frmInformeMuestrasAnalizadasPorAnalistaDeterminacion.frx":08CA
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   8685
      Width           =   1590
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame4 
         ...
         Begin VB.OptionButton opUrgente 
            Appearance      =   0  'Flat
            BackColor       =   &H00C0C0C0&
            Caption         =   "Todo Sin Aguas y Baños"
            ForeColor       =   &H80000008&
            Height          =   240
            Index           =   2
            Left            =   3060
   ...
   End
End
Attribute VB_Name = "frmInformeMuestrasAnalizadasPorAnalistaDeterminacion"
    ...
    Else
        ...
        For i = 1 To lista.ListItems.Count
            If InStr(lista.ListItems(i).Text, "TOTAL") = False Then
                ...
                XLS.Cells(fila, 4) = lista.ListItems(i).SubItems(3)
                XLS.Range(XLS.Cells(fila, 4), XLS.Cells(fila, 4)).HorizontalAlignment = xlCenter
                fila = fila + 1
            End If
        Next
        XLA.visible = True
...
cmdMarcar_Click_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure cmdMarcar_Click of Formulario frmInformeMuestrasAnalizadasPorAnalistaDeterminacion", vbCritical, App.Title
End Sub
...
Private Sub cabecera()
    ...
    With lista.ColumnHeaders
        .Add , , "Analista", 3500, lvwColumnLeft
        .Add , , "Tipo Determinación", 3500, lvwColumnLeft
        .Add , , "PNT", 3500, lvwColumnLeft
...
Private Sub buscar()
    ...
    If opUrgente(1).Value = True Then
        filtro = filtro & " and m.tipo_muestra_id in (2,6)"
    End If
    If opUrgente(2).Value = True Then
        filtro = filtro & " and m.tipo_muestra_id not in (2,6)"
    End If
    
    consulta = "SELECT CONCAT(c.NOMBRE,' ',c.APELLIDOS),e.NOMBRE, ifnull(f.NOMBRE,''),COUNT(*) " & _
                " FROM muestras m " & _
                " inner join determinaciones d on m.ID_MUESTRA = d.MUESTRA_ID " & _
                " inner join usuarios c on d.EMPLEADO_ID = c.ID_EMPLEADO " & _
                " inner join tipos_determinacion e on d.TIPO_DETERMINACION_ID = e.ID_TIPO_DETERMINACION  " & _
                " left join ca_documentos f on e.PNT_VINCULADO = f.ID_DOCUMENTO " & _
               " WHERE m.ANULADA = 0 and d.EMPLEADO_ID <> 0 " & _
               filtro & strMuestra & strCliente & strCentro & strTE & strSECTOR & _
               fecha_desde & fecha_hasta & _
               " GROUP BY CONCAT(c.NOMBRE,' ',c.APELLIDOS),e.NOMBRE, f.NOMBRE ORDER BY CONCAT(c.NOMBRE,' ',c.APELLIDOS) "
    Me.MousePointer = 11
