...
Begin VB.Form frmPC_Facturacion 
   BackColor       =   &H00C0C0C0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Lotes de Productos Controlados pendientes de facturación"
   ClientHeight    =   9930
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   14355
   Icon            =   "frmPC_Facturacion.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Frame frmFacturaManual 
      ...
      Begin XtremeSuiteControls.PushButton cmdCerrarFacturaManual 
         ...
         Caption         =   "Cerrar"
         Appearance      =   5
         Picture         =   "frmPC_Facturacion.frx":030A
      End
      Begin XtremeSuiteControls.PushButton PushButton2 
         ...
         Caption         =   "Informar"
         Appearance      =   5
         Picture         =   "frmPC_Facturacion.frx":6B6C
      End
   ...
   Begin VB.CommandButton cmdCrear 
      ...
      Left            =   10080
      MaskColor       =   &H00FFFFFF&
      Picture         =   "frmPC_Facturacion.frx":D3CE
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   8970
      UseMaskColor    =   -1  'True
   ...
   Begin VB.CommandButton cmdCrear 
      ...
      Left            =   11520
      MaskColor       =   &H00FFFFFF&
      Picture         =   "frmPC_Facturacion.frx":E010
      Style           =   1  'Graphical
      TabIndex        =   6
      Top             =   8970
      UseMaskColor    =   -1  'True
   ...
   Begin XtremeSuiteControls.PushButton cmdNoFacturable 
      ...
      Appearance      =   5
      Picture         =   "frmPC_Facturacion.frx":EC52
   End
   Begin VB.Label lblCampos 
      Alignment       =   2  'Center
      AutoSize        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmPC_Facturacion"
...
Private Sub Command1_Click()
    ...
    If lista.ListItems.Count > 0 Then
        LOTE = lista.ListItems(lista.selectedItem.Index).SubItems(5)
        For i = 1 To lista.ListItems.Count
            If lista.ListItems(i).SubItems(5) = LOTE Then
...
Private Sub rellenar_clientes()
    ...
    If CrearConexionGlobal(conn, "", "") = True Then ' CONECTAR EL RS
        consulta = "SELECT DISTINCT C.ID_CLIENTE,C.NOMBRE " & _
                   "  FROM PC_PEDIDO PC, CLIENTES C " & _
                   " WHERE PC.CLIENTE_ID = C.ID_CLIENTE " & _
                   "   AND PC.DOC_ID = 0 "
        With cmbclientes
            .setCONN = conn
            .setQUERY = consulta
...
Private Function inlote() As String
    ...
    For i = 1 To lista.ListItems.Count
        If lista.ListItems(i).Checked = True Then
            s = s & lista.ListItems(i).SubItems(4) & ","
        End If
    Next
    s = Left(s, Len(s) - 1)
    inlote = s
...
'                         End If
                ...
                With oConcepto
                        ...
                        .setFAMILIA_ID = familia.productos_controlados ' Familia de alodine por defecto
                        .setAPARTADO = 0
                        .setDTO = 0
                        ...
                        End If
                End With
                oPC.doc_pago rs(7), CLng(num_doc)
                rs.MoveNext
...
generar_documentos_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure generar_documentos of Formulario frmPC_Facturacion"
End Sub
...
Private Sub cargar_lista()
    ...
    Set rs = oPC.Listado_Pendientes_Facturar(IIf(cmbclientes.getTEXTO = "", 0, cmbclientes.getPK_SALIDA), fdesde, fhasta)
    If rs.RecordCount <> 0 Then
        Do
...
Private Sub actualizarLista()
    Dim rs As New ADODB.Recordset
    Dim oPC As New clsPc_pedido
    Set rs = oPC.Listado_Pendientes_Facturar_ID(lista.ListItems(lista.selectedItem.Index).SubItems(4))
    If rs.RecordCount <> 0 Then
        Do
...
Private Sub lista_DblClick()
    If lista.ListItems.Count > 0 Then
        frmPC_Detalle.PK = lista.ListItems(lista.selectedItem.Index).SubItems(4)
...
Private Sub PushButton2_Click()
   ...
   On Error GoTo PushButton2_Click_Error
    ...
    Else
        ...
        For i = 1 To lista.ListItems.Count
            If lista.ListItems(i).Checked = True Then
                oPC.doc_pago lista.ListItems(i).SubItems(4), ID
            End If
        Next
...
PushButton2_Click_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure PushButton2_Click of Formulario frmPC_Facturacion"
End Sub
