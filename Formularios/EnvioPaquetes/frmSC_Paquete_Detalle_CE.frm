...
Begin VB.Form frmSC_Paquete_Detalle_CE 
   BackColor       =   &H00C0C0C0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Subcontratación de ensayos - Detalle de paquete"
   ClientHeight    =   8730
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   12630
   Icon            =   "frmSC_Paquete_Detalle_CE.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.Frame Frame1 
      ...
      Begin MSDataListLib.DataCombo cmbCentro 
         Bindings        =   "frmSC_Paquete_Detalle_CE.frx":08CA
         Height          =   315
         Left            =   7740
         TabIndex        =   50
         Top             =   945
   ...
   Begin VB.Label lblTramite 
      BackColor       =   &H80000009&
      Caption         =   "Necesita permisos como tramitador si desea modificar el paquete"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmSC_Paquete_Detalle_CE"
...
Private Sub cmdFacturacion_Click()
        ...
        frmProveedores_Facturas.TOBJETO = TOBJETO.TOBJETO_SC_EFICACIA
...
Private Sub cmdAceptar_Click()
    ...
    lstMuestras.ListItems(lstMuestras.selectedItem.Index).SubItems(7) = moneda(Trim(txtNuevoPrecio.Text))
...
Private Sub cmdEliminar_Click()
    If lstMuestras.ListItems.Count = 0 Then
        Exit Sub
    End If
    If MsgBox("¿Desea eliminar el ensayo?", vbQuestion + vbYesNo, App.Title) = vbYes Then
    ...
    If datos_correctos Then
        ...
        If MsgBox("Va a modificar el paquete. ¿Está seguro?", vbQuestion + vbYesNo, App.Title) = vbYes Then
            ...
            For i = 1 To lstMuestras.ListItems.Count
                opd.CargaCE PK, EDICION, lstMuestras.ListItems(i).SubItems(5), lstMuestras.ListItems(i).SubItems(6)
                opd.setPRECIO = moneda_bd(Trim(lstMuestras.ListItems(i).SubItems(7)))
...
cmdok_Click_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure cmdOk_Click of Formulario frmSC_Paquete_Detalle_CE"
End Sub
...
Private Sub lstMuestras_Click()
    ...
    If lstMuestras.ListItems.Count > 0 Then
        ...
        txtNuevoPrecio.ForeColor = vbBlue
        txtNuevoPrecio.Text = moneda((lstMuestras.ListItems(lstMuestras.selectedItem.Index).SubItems(7)))
    End If
...
Private Sub lstMuestras_DblClick()
   On Error GoTo lstMuestras_DblClick_Error
    ...
    If lstMuestras.ListItems.Count > 0 Then
        ...
        Else
            gmuestra = lstMuestras.ListItems(lstMuestras.selectedItem.Index).SubItems(5)
...
lstMuestras_DblClick_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure lstMuestras_DblClick of Formulario frmSC_Paquete_Detalle_CE"
...
Public Sub CARGAR()
    ...
    Dim lngTotalEnsayosPaquete As Long
...
'M1274-I
            If .getEDICION = 0 Then
                txtEdicion = 1   'para prevenir del caso en que EDICION no esté informado (no debe ocurrir porque en la instalación se ejecuta el UPDATE al valor 1)
            Else
                txtEdicion = .getEDICION 'CE y DETER solo contempla modificaciones sin cambio de versión
            End If
'M1274-F
            ...
            'M1274-I
            'Set RS = oSC_Paquete.Listado_muestras_ensayos(.getID_PAQUETE)
            Set rs = oSC_Paquete.Listado_muestras_ensayos(.getID_PAQUETE, EDICION)
            'M1274-F
            lngTotalEnsayosPaquete = rs.RecordCount
            If rs.RecordCount <> 0 Then
                Do
                    With lstMuestras.ListItems.Add(, , rs(0)) ' Código
                        .SubItems(1) = rs(1)                  ' ref Cliente
                        .SubItems(2) = rs(2)                  ' Tipo de ensayo
                        .SubItems(3) = rs(3)                  ' Valor de referencia
                        .SubItems(4) = rs(4)                  ' Normativa aplicable
                        .SubItems(5) = rs(5)                  ' ID_MUESTRA
                        .SubItems(6) = rs(6)                  ' ID_TIPO_ENSAYO
                        .SubItems(7) = moneda(rs(7))                 ' PRECIO
    ...
    txtSuma = SumarImportes
    txtDatos(2) = txtSuma
    lblSubtitulo(2) = lblSubtitulo(2) & " - Nº ENSAYOS: " & lngTotalEnsayosPaquete
End Sub
...
Private Function SumarImportes() As String
    ...
    For indice = 1 To lstMuestras.ListItems.Count
        Suma = Suma + CDbl(lstMuestras.ListItems(indice).SubItems(7))
    Next indice
