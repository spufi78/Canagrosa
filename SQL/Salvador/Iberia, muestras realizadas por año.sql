select a.ID_GENERAL,a.ANNO,concat(c.CODIGO,'-',a.ID_particular) as codigo, a.FECHA_RECEPCION,a.TIPO_MUESTRA_ID,
       ...
       if(e_bond.MICROESTRUCTURA1=1,1,0) AS BOND_MICROESTRUCTURA,
		 if(e_bond.TRACCION<>'',1,0) AS BOND_TRACCION,
		 if(e_bond.MACRO_DUREZA<>'',1,0) AS BOND_MACRO_DUREZA,
		 if(e_bond.MICRO_DUREZA<>'',1,0) AS BOND_MICRO_DUREZA,
	  	 if(e_bond.ESPESOR<>'',1,0) AS BOND_ESPESOR,
       if(e_top.MICROESTRUCTURA1=1,1,0) AS TOP_MICROESTRUCTURA,
		 if(e_top.TRACCION<>'',1,0) AS TOP_TRACCION,
		 if(e_top.MACRO_DUREZA<>'',1,0) AS TOP_MACRO_DUREZA,
		 if(e_top.MICRO_DUREZA<>'',1,0) AS TOP_MICRO_DUREZA,
	  	 if(e_top.ESPESOR<>'',1,0) AS TOP_ESPESOR,
	  	 if(a.TIPO_MUESTRA_ID IN (47,200),count(tdet.ID_TIPO_DETERMINACION),0) AS COMBUSTIBLES,
	  	 ...
	  	 if(a.TIPO_MUESTRA_ID NOT IN(222,47,200,24,6,160,80,98,118,55),1,0) AS OTROS
 from muestras a 
