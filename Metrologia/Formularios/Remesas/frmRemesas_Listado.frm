...
Private Sub cmdEliminar_Click()
    If lista.ListItems.Count > 0 Then
        If MsgBox("¿Desea eliminar realmente la remesa?", vbYesNo + vbQuestion, App.Title) = vbYes Then
            Dim oRemesa As New clsRemesas
            oRemesa.Eliminar lista.ListItems(lista.SelectedItem.Index).Text
            Set oRemesa = Nothing
            cargar_lista
        End If
    End If
...
Private Sub cmdImprimir_Click(Index As Integer)
   ...
   On Error GoTo cmdImprimir_Click_Error
    ...
    If cmbBanco.getTEXTO <> "" Then
        FILTRO = FILTRO & " AND {remesas.BANCO_ID} = " & cmbBanco.getPK_SALIDA
    End If
    If txtDatos(0) <> "" Then
        If IsNumeric(txtDatos(0)) Then
            FILTRO = FILTRO & " AND {remesas.ID_REMESA} = " & txtDatos(0)
        End If
    End If
    
    FILTRO = FILTRO & " AND {remesas.FECHA} in Date (" & Year(fdesde) & "," & Month(fdesde) & "," & Day(fdesde) & ") to Date (" & Year(fhasta) & "," & Month(fhasta) & "," & Day(fhasta) & ")"
    ...
    With frmReport
        ...
        .CRITERIO = FILTRO
        If Index = 0 Then
            .informe = "rptremesas_listado"
        Else
            .informe = "rptremesas_listado_desglosado"
        End If
        .ParametrosNombre = p1
...
Private Sub cmdModificar_Click()
    If lista.ListItems.Count > 0 Then
        frmRemesas_Detalle.pk = lista.ListItems(lista.SelectedItem.Index).Text
        frmRemesas_Detalle.Show 1
        actualizar_lista
    End If
End Sub
...
Public Sub cargar_lista()
    ...
    If txtDatos(0) <> "" Then
        If IsNumeric(txtDatos(0)) Then
            FILTRO = FILTRO & " AND A.ID_REMESA = " & txtDatos(0)
        End If
    End If
    Dim rs As New ADODB.Recordset
    consulta = "SELECT A.ID_REMESA, A.FECHA, C.DESCRIPCION, D.USUARIO, COUNT(*), SUM(IMPORTE)" & _
               "  FROM REMESAS A" & _
               "  LEFT JOIN REMESAS_DOCUMENTOS B ON A.ID_REMESA = B.REMESA_ID " & _
               "  LEFT JOIN DECODIFICADORA C ON A.BANCO_ID = C.VALOR AND C.CODIGO = 9 " & _
               "  LEFT JOIN USUARIOS D ON A.USUARIO_ID = D.ID_EMPLEADO " & _
               " WHERE A.FECHA >= '" & Format(fdesde, "YYYY-MM-DD") & "'" & _
               "   AND A.FECHA <= '" & Format(fhasta, "YYYY-MM-DD") & "'" & _
               FILTRO & _
               "  GROUP BY A.ID_REMESA, A.FECHA, C.DESCRIPCION,D.USUARIO" & _
               "  ORDER BY A.ID_REMESA DESC "
    ...
    If rs.RecordCount >= 1 Then
        ...
        While Not rs.EOF
                ...
                With lista.ListItems.Add(, , Format(rs(0), "0000")) ' Remesa
...
Private Sub actualizar_lista()
   ...
   On Error GoTo actualizar_lista_Error
    ...
    consulta = "SELECT A.ID_REMESA, A.FECHA, C.DESCRIPCION, D.USUARIO, COUNT(*), SUM(IMPORTE)" & _
               "  FROM REMESAS A" & _
               "  LEFT JOIN REMESAS_DOCUMENTOS B ON A.ID_REMESA = B.REMESA_ID " & _
               "  LEFT JOIN DECODIFICADORA C ON A.BANCO_ID = C.VALOR AND C.CODIGO = " & DECODIFICADORA.D_BANCOS & _
               "  LEFT JOIN USUARIOS D ON A.USUARIO_ID = D.ID_EMPLEADO " & _
               " WHERE A.ID_REMESA = " & lista.ListItems(lista.SelectedItem.Index) & _
               "  GROUP BY A.ID_REMESA, A.FECHA, C.DESCRIPCION,D.USUARIO" & _
               "  ORDER BY A.ID_REMESA DESC "
    Dim ID As Long
    Set rs = datos_bd(consulta)
...
actualizar_lista_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure actualizar_lista of Formulario frmRemesas_Listado"
End Sub
